...
font = ImageFont.truetype('fonts/simkai.ttf', 30)
...
def plot_one_box_chinese(x, img, color=None, label=None, line_thickness=None):
    # Plots one bounding box on image img
    tl = line_thickness or round(0.002 * (img.shape[0] + img.shape[1]) / 2) + 1  # line/font thickness
    color = color or [random.randint(0, 255) for _ in range(3)]
    c1, c2 = (int(x[0]), int(x[1])), (int(x[2]), int(x[3]))
    ...
    if label:
        ...
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        draw.rectangle((c1[0], c1[1], c2[0], c2[1]), (color[2], color[1], color[0]))
        draw.text((c1[0], c1[1]-28), label, (255, 255, 255), font=font)
...
def tracking_line(im0, color=None, objtracking=None):
    color = color or (0,255,0) #[random.randint(0, 255) for _ in range(3)]
    if objtracking:
        for i in range(0, len(objtracking) - 1):
            centroid, t = objtracking[i]
            if (time.time() - t > 10):
                continue
            centroid_next, _ = objtracking[i + 1]
            cv2.line(im0, (centroid[0], centroid[1]), (centroid_next[0], centroid_next[1]), color, 3)
    return im0
