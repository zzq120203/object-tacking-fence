...
from kalman.kalman_filter_backup import KalmanFilter
from kalman.common import dprint
from kalman.ukf import UKF
...
class Track(object):
    ...
    def __init__(self, kobj, trackIdCount):
...
class Tracker(object):
    ...
    def Update(self, kobjs, flag):
        detections = [kobj.center for kobj in kobjs]
        ...
        if flag==1:
            ...
            if len(self.tracks) == 0:
                for i in range(len(detections)):
                    ...
                    track = Track(kobjs[i], self.trackIdCount)
                    self.trackIdCount += 1
            ...
            if len(un_assigned_detects) != 0:
                for i in range(len(un_assigned_detects)):
                    track = Track(kobjs[un_assigned_detects[i]],
                                  self.trackIdCount)
                    self.trackIdCount += 1
                    self.tracks.append(track)
            ...
            for i in range(len(assignment)):
                ...
                if assignment[i] != -1:
                    self.tracks[i].kobj.xyxy = kobjs[assignment[i]].xyxy
